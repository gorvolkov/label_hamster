...
class Stamper:
    ...
    def _stamp_a4(self, label: Label):
        # create template pdf
    ...
    def _stamp_7x5_old(self, label: Label):
    ...
    def _stamp_7x5(self, product: ProductRaw):
        # Размер страницы 7 см по ширине и 5 см по высоте (альбомная ориентация)
        page_width, page_height = 70, 50  # мм
    ...
    def _stamp_6x4(self, format: str | None = None):
        ...


    def stamp(self, product: ProductRaw, format: str|None=None):
        if format is None:
            stamp_func = self.formats["7x5"]
        else:
